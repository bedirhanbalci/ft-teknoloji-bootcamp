    ...
    public static Specification<Blog> initSpecification(BlogSearchRequest request) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (request.getTitle() != null) {
                predicateList.add(criteriaBuilder.like(root.get("title"), "%" + request.getTitle() + "%")); // column name değil! attribute name olmalı!
            }

            if (request.getLikeCount() != 0) {
                predicateList.add(criteriaBuilder.greaterThanOrEqualTo(root.get("likeCount"), request.getLikeCount()));
            }
